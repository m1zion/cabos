...
function ExperienceDetail() {  
    ...
    return (
       ...
       {/*} <div className="w-[100%] sm:w-[100%] xl:w-[100%] mb-[3rem]" style={{ backgroundImage: "url('./public/assets/icons/tortoise-shell.svg')" }}>*/}
            ...
            <div className="flex flex-col">
                <span className="font-medium text-lg text-[#728E9F] flex items-center gap-1"><CalendarDaysIcon aria-hidden="true" className="shrink-[0] block size-5"/>{content.period}:</span>
                <span>{t.period}</span>
            </div>
            <div className="flex flex-col">
                <span className="font-medium text-lg text-[#728E9F] flex items-center gap-1"><UserGroupIcon aria-hidden="true" className="shrink-[0] block size-5"/>{content.age}:</span>
                <span>{t.age}</span>
            </div>
            <div className="flex flex-col">
                <span className="font-medium text-lg text-[#728E9F] flex items-center gap-1"><CurrencyDollarIcon aria-hidden="true" className="shrink-[0] block size-5"/>{content.price}:</span>
                <span>{t.price}</span>
            </div>
            <div className="flex flex-col">
                <span className="font-medium text-lg text-[#728E9F] flex items-center gap-1"><TruckIcon aria-hidden="true" className="shrink-[0] block size-5"/>{content.rounTripTransportation}:</span>
                <span>{t.rounTripTransportation}</span>
            </div>
            {t.additionalData && (
                <div className="flex flex-col sm:col-span-2">
                <span className="font-medium v text-[#728E9F]">📌 {content.additionalData}:</span>
